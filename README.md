...
- **Frontend** — Next.js (App Router) + TypeScript, statically exported: submit an email request, get instant
  validation feedback, and watch its status change live. (The API accepts SMS and PUSH too; the form keeps to
...
- **No authentication or rate limiting.** The API is open. This is a demonstration of the processing
  pipeline; a real deployment would put a JWT authorizer (Cognito) and WAF rate rules in front of it.
...
3. **Authentication** — Cognito user pool + HTTP API JWT authorizer; scope the list endpoint to the caller
   (`ownerId` becomes the GSI partition key, which also fixes the single-partition note above).
