...
| Data     | DynamoDB `notification-requests-dev` with GSIs `byCreatedAt` and `byUser`                                             | PAY_PER_REQUEST, point-in-time recovery on.                                                                                                                                               |
...
| ------------ | ---------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| **Receive**  | API Gateway HTTP API → `createNotification` Lambda (`backend/src/handlers/http/createNotification/`) | Accepts `POST /notifications`; API Gateway answers CORS preflights, `httpHandler` rejects bodies over 32 KB before parsing.                                                                                                                                                                                                                           | `413 PAYLOAD_TOO_LARGE`                                                          |
...
| **Handlers → services → repositories/queue/providers** layering (full design: [docs/lambda-function-design.md](docs/lambda-function-design.md))  | Handlers only parse input and map output; services hold the business rules and receive their dependencies through a factory, so they are unit-testable with `jest.fn()` mocks and never import the AWS SDK.                                                                          | Everything in the handler: shorter, but the lifecycle rules would be untestable without AWS mocks.                                                                  |
