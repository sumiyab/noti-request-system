...
└── src/
    ...
    │   │   └── ListFooter.tsx           "Showing N · more available" + Load more (cursor page); page size 10
...
| `NotificationList`                 | loading / empty / error states, mapping pages to rows, the footer ("Showing N · more available" / "Showing all N") with Load more, the user filter (debounced 300 ms, validated with `userIdSchema`, passed to `useNotifications`) | polling logic (in the hook)                            |
