...
| Components                | shadcn/ui                                                           | Copied into `src/components/ui/`, not a dependency: Button, Input, Textarea, Label, RadioGroup, Form, Badge, Card, Sonner. Accessible (Radix) and ours to edit.                                      |
...
| Lint                      | ESLint 9 (flat config) + `typescript-eslint` + `eslint-config-next` | Rules below.                                                                                                                                                                                         |
...
└── src/
    ...
    │   │   └── MessageFields.tsx        subject (unregistered for SMS) + message with counter
    │   ├── notification-list/
...
| `ChannelField` / `RecipientField` / `MessageFields` | markup via shadcn `FormField`, per-channel labels/hints/limits, character counters from `watch()`                                                                                        | hold state (they receive `control`)                    |
...
| shadcn `Form*`                                      | label, control slot, description, error text, `aria-invalid` / `aria-describedby` wiring — generated once, reused by every field                                                         | validation                                             |
...
- **Discriminated union and the subject field.** `subject` exists in `defaultValues` for EMAIL/PUSH; when
  `channel` becomes `SMS` the `MessageFields` component calls `form.unregister('subject')`, so the submitted
  object has no `subject` key and the strict SMS schema accepts it. Switching back re-registers it.
...
radio-group form badge card sonner`. They live in `src/components/ui/`, are linted and formatted like any
...
export default [...base, ...nextVitals, { files: ['**/*.{ts,tsx}'], rules: { 'react/display-name': 'off' } }];
```
...
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------- |
| `tests/lib/api.test.ts`                      | 202 → returns `data`; 400 → throws `ApiError` with `code` and `details`; network failure → `ApiError('NETWORK_ERROR')`; `Location` handling                                                                                                                        | `fetch` mocked with `Response` objects                              |
| `tests/hooks/useNotifications.test.tsx`      | polls while an item is `QUEUED`, stops when all terminal; `nextCursor` → `fetchNextPage`                                                                                                                                                                           | `renderHook` with a fresh `QueryClient` per test; fake timers       |
| `tests/hooks/useCreateNotification.test.tsx` | success prepends into the cache and invalidates; error surfaces on `mutation.error`                                                                                                                                                                                | same                                                                |
| `tests/components/NotificationForm.test.tsx` | switching to SMS removes the subject field from the DOM and the submitted values; invalid email shows the schema's message on blur and blocks submit; server `VALIDATION_ERROR` lands on the right `FormMessage`; submit disabled while pending; resets on success | `render` + `userEvent`, `fetch` mocked; real `useForm` and resolver |
| `tests/components/NotificationList.test.tsx` | loading, empty, error, rows with badges; `lastError` shown on `FAILED`; load-more appears only with a cursor                                                                                                                                                       | `render` inside a `QueryClientProvider` with pre-seeded cache       |
| `tests/components/StatusBadge.test.tsx`      | label/colour per status; `role="status"`                                                                                                                                                                                                                           | `render`                                                            |
